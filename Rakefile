...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/crawler/stimulus_tag_helper/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("stimulus_tag_helper.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in stimulus_tag_helper.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "stimulus_tag_helper.gemspec", /ruby_version = .*">= (.*)"/ => lowest
